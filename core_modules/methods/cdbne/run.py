...
class CDBNE(nn.Module):
    def __init__(self, num_features, middle_size, representation_size, alpha, clusters_number, v=1):
        ...
        pretrain_path = Path(args.pathes)
        if pretrain_path.is_file():
            try:
                gate_state = torch.load(pretrain_path, map_location='cpu', weights_only=True)
            except TypeError:
                gate_state = torch.load(pretrain_path, map_location='cpu')
            self.gate.load_state_dict(gate_state)
            print(f"Loaded pretrained weights: {pretrain_path}")
        else:
            print(f"Pretrained weights not found: {pretrain_path}. Train from scratch.")
...
def trainer(shujuji):
    ...
    # Modula is implemented with NumPy, so keep a CPU copy of adj.
    ...
    for epoch in range(args.max_epoch):
        ...
        mu_value = Modula(adj_np, A_pred.detach().cpu().numpy())
        MU_loss = torch.as_tensor(mu_value, device=device, dtype=re_loss.dtype)
...
if __name__ == "__main__":
    ...
    args.pathes = f'./pre/preCDBNE_{args.name}.pkl'
